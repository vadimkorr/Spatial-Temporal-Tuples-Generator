...
var conv 		= require("./scripts/conv.js")
...
function genAndSave(fileName, numberOfTuples, linesPortion, segRangeInLine, latOffsetRange, lngOffsetRange, radiusRange, clusterIdRange, toFixed, minTime, maxTime, daysInterval) {
    ...
    for (i = 0; i < numberOfTuples; i++) {
		...
		var clusterId = geo.getRandomIntInRange(clusterIdRange);
		var timeInterval = time.getRndTimeInterval(new Date(minTime), new Date(maxTime), conv.daysToMs(daysInterval), true)
		var str = geo.getStr(i, wkt, uncRadius, timeInterval[0], timeInterval[1], "", clusterId, ";");
...
init();
genAndSave(fileName, numberOfTuples, linesPortion, segRangeInLine, latOffsetRange, lngOffsetRange, radiusRange, clusterIdRange, toFixed, minTime, maxTime, daysInterval);